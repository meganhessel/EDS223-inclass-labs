...
# data frame with spatial column 
# Use tidyverse 
...
```{r}
# Check geometry type - polygon or line ? 
...
```{r}
# Update park boundarty object name to clarify its a polygon 
park_polygon <- boundary 

# Create line version of park boundary 
# CASE TO CONVERT polygon to a line 
...
# Transfrom to porjected crs 
cycle_hire_osm_projected <- st_transform(cycle_hire_osm, "EPSG:27700")

# Define raster template for cycle data (1000 meters)
...
# True color imagery 
map1 <- tm_shape(L7) + 
  tm_rgb(col = tm_vars(c(3, 2, 1), multivariate = TRUE)) + # Define multiple bands 
  tm_title(text = "True color")

# False color imagery - emphasizing 
map2 <- tm_shape(L7) + 
  tm_rgb(col = tm_vars(c(4, 3, 2), multivariate = TRUE)) + # Define multiple bands 
  tm_title(text = "NIR, red, green")

# False color imagery- emphasizing monitoring floods 
map3 <- tm_shape(L7) + 
  tm_rgb(col = tm_vars(c(5, 4, 2), multivariate = TRUE)) + # Define multiple bands 
  tm_title(text = "SWRIR, NIR, Green")
...
# appli ndvi function to landsat 3 and 4 
...
# Apply NDWI function to landsat bands 3 and 4 
ndwi_rast <- lapp(L7[[c(2,4)]], fun = ndwi_fun)


ndwi_map <- tm_shape(ndwi_rast) + 
  tm_raster(
...
tmap_arrange(map1, map2, ndwi_map)
```


Normalize Difference Moisture Index: vegeation indeix used in RS to measure and monitor the water content in vegeation 

NDMI = (nir - swir) / (nir + swir)

measuring moisture in plant levels 
higher values in more water content in vegetation 
...
tmap_arrange(map1, map2, ndwi_map, ndmi_map)

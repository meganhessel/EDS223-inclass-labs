...
tm_shape(world_vector) +  # Defines the spatial object 
...
```{r}
# ORDER MATTERS!! PLOTTING IN ORDER WE ARE CODING IT 
...
```{r}
# Plotting vector and raster spatal objects 

tm_shape(world_elevation)+ #Raster data of global elevations 
  tm_raster()+
tm_shape(world_vector) + # Outline of countries 
  tm_borders() + 
tm_shape(world_cities) + # Added cities with dots and lables 
  tm_dots() + 
  tm_text("name", size = 0.7, col = "blue") # Add text from what column 
...
tm_shape(world_vector) + 
  tm_polygons() + 
tm_shape(world_cities, is.main = TRUE) + # Ploting world cities last BUT set as main shape
  tm_dots() + 
  tm_text("name")
```
...
```{r}
tm_shape(world_elevation, bbox = c(-15,35,45,65)) + # adding bounding box to restrict extent of map 
  tm_raster() + 
tm_shape(world_vector) + 
  tm_borders()
```
...
```{r}
tm_shape(world_elevation, bbox = world_cities) + # bounding box is the extent of world cities 
  tm_raster() + 
tm_shape(world_vector) + 
  tm_borders()
...
```{r}
tm_shape(world_vector) + 
  tm_polygons(fill= "lightblue", # color inside 
              col = "black", # Border color 
              lwd = 0.5, # Line width 
              lty = "dashed" # Line type ll
              )
...
```{r}
# Fill polygone with fixed colors 
tm_shape(world_vector) + 
...
tm_shape(world_vector) + 
  tm_polygons(fill = "life_expectancy", 
              ...
              fill.legend = tm_legend(title = "Life Expectancy (yrs)")
              )
...
tm_shape(world_vector) + 
  tm_polygons(fill = "life_expectancy", 
              ...
              # "brewer.YlOrBr"
              # "Pastel1"
              fill.legend = tm_legend(title = "Life Ex (yrs)"))
```
...
tm_shape(world_vector)+ 
  tm_fill()+ 
  tm_scalebar() + 
  tm_compass(north = 270, position = c("left", "top"))
```
...
tm_shape(world_vector) + 
  tm_polygons(fill = "gdp_per_cap", 
              fill.scale = tm_scale(breaks = c(0,500,1000,2000,4000,80000), 
                                    lables = c('low', 'medium', 'high', 'very high'))) + 
  tm_title(text = "Global Income", 
           frame = TRUE) + 
  tm_layout(text.frontfamily = "Ariel", 
            style = "beaver", 
...
tmap_save(map_1, here::here("week1", "tmap_example.png"))

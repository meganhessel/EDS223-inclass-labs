...
```{r}
# To look at the data, just hav eto plot it 
...
```{r}
# Output will have 4 different layers to show 4 dif bands 
...
# Merge austria ans switzerland raters 
merge <- merge(austria, switzerland)
  # Need to be similar in extent, position, and resolution 
```

```{r}
map1 <- tm_shape(austria) + 
  tm_raster(col.legend = tm_legend("Elevation(m)"), 
            col.scale = tm_scale_intervals(breaks = c(0,1000,2000,3000, 4000, 5000)))+ 
...
# Row 2, column 3 - (diminstions = 465 -->  465 (row 1) + 3 = 468) 
zion_elevation[2, 3]
```
...
```{r}
# Replacing values in multi layer with a matrix 
...
# Check NA - now there is 1 NA because we just made it with a condition 
global(is.na(test_raster), "sum")
...
```{r}
# Create a point within areas covered by raster 

# pt1 define coordinates 
point <- matrix(c(-113, 37.5), ncol = 2)


# pt2 EXTRACT FROM RASTER DATA 


# METHOD 1: find cell id 
id <- terra::cellFromXY(zion_elevation, xy = point)
# Index to cell - returns elevation that is at the location 
zion_elevation[id]


# METHOD 2: using terra::extract 
...
```{r}
# Find extend/bounding box of current raster
terra::ext(zion_elevation)

# Create raster with smaller extend 
clip <- rast(xmin = -113.2, xmax = -113, ymin = 37.2, ymax = 37.9, 
             resolution = 0.3, # resolution = size of cell 
...
```{r}
# Clipping removing cells 
zion_elevation_clip <- zion_elevation[clip, 
                                      drop = FALSE] # drop = TRUE, drops the geometeries 

```

```{r}
# Mapping original extent vs clipped extent 
map1 <- tm_shape(zion_elevation) + 
...
```{r}
# Message about extents matching 
...
Select observations that meet the parameter = the mask

```{r}
# Create raster mask of the same resolution and extent  
rmask <- zion_elevation

# Set all cells with elevation < 2,000 meters to NA 
rmask[rmask < 2000 ] <- NA 
     # If working with different layers, need to specify what layer you are trying to access 
```

masking: THEN subset the elevation raster to the selected values (mask)
...
# Approach 2: Used mask function 
masked2 <- mask(zion_elevation, rmask) # (What u are masking, what u want to USE to mask)
...
zion_elevation^2
log(zion_elevation)
zion_elevation > 5 # Determins if each cell has a value greater than 5 
...
Reclassifying our legend groupings with a matrix with 4 groups
...
# Use reclass matrix to reclassify elevation raster with `classify()`
reclassified <- classify(zion_elevation, rcl = rcl) # (Raster, rcl = matrix)


# Change relcassified values into factors 
# Look at factors as a group 
values(reclassified) <- as.factor(values(reclassified))
  # If don't do this, summary()  will take centeral tendencies of group #s 



# Check if worked 
summary(reclassified)
  # Tells you how many values in each group 
  # 0: first cell we changed in the beginning 
```

```{r}
map1 <- tm_shape(zion_elevation) + 
  tm_raster(co.legend = tm_legend(title = "Elevation (m)")) + 
  tm_title(text = "Orginal")
...
ndvi - used to look at density vegitation - 1 = dense vegitation, -1 = no vegetation
...
# apply ndvi function to landsat bands 3 & 4 
  # lapp() - apply a given function to 
  # 4 layer = nir layer 
  # 3 layer = red layer 
...
```{r}
# Look at min of each 9 by 9 box and assigning that value to the matrix 
elevation_focal <- focal(zion_elevation, 
                         w = matrix(1, nrow = 9, ncol = 9), # 1 = look at each cell individually 
                         fun = min ) 

# Smoothed out the elevation gradient, as alike neighbors now have the same value 
```
...
```{r}
zonal(zion_elevation, reclassified, fun = "mean") %>% # (df, use as zional grouping, function) 
...
```{r}
# Changing extend - adding empty columns 
elev_2 <- extend(zion_elevation, c(1, 200)) 
# Add 1 row to top and bottom ee
# Add 200 columns to left and right 
```

Performing algebraic operations on objects with different extents doesn't work.

```{r}
# ERROR: extends dont match  --> Cannot preform opperations 
zino_elevation + elev_2
```
...
```{r}
# Making elev_3 the same extent of elev_2 
elev_3 <- extend(zion_elevation, elev_2) 

# Now can preform operations because extends are the same 
elev_2 + elev_3
```
...
```{r}
# Aggregating 
# Decreasing resolution / decreasing number of cells = change shape 
zion_elevation_coarse <- aggregate(zion_elevation, fact = 5, fun = mean)
# Cells are now larger 
# Different than neighboring because in neighboring we were just taking the mean of neighboring cells; here are CHANGING resolution 
```
...
```{r}
# Spiting each pixel into a 5 by 5 grid / cells
zion_elevation_disagg <- disagg(zion_elevation_coarse, fact = 5, method = "bilinear")

# DOES NOT MATCH THE ORGINAL DATA BECAUSE YOU CAN NEVER GET BACK TO ORGINAL DATA WHEN DISAGGREGATING 

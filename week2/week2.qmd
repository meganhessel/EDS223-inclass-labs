...
london_point # Just a point 

# Add a CRS 
london_geom <- st_sfc(london_point, crs = 4326)
london_geom

class(london_geom) # POINT 
...
# check the class - will return: "sf" "dataframe"
class(london_sf)
...
```{r}
st_crs(london_sf)
# Coordinate reference system is exactly what we wanted it to be - 4326 
```
...
```{r}
# STICKY DATA!! 
# Cannot remove the geom column the normal way 
world_df <- world %>% 
...
```{r include=TRUE}
# NOW, the geom column is dropped 
world_df <- st_drop_geometry(world) # only way to get rid of the geom column 
...
```{r}
st_crs(london_proj)
# crs is exactly what we wanted: EPSG:27700 
```
...
```{r}
# The projected and geoghraphic projection DO NOT match 
...
```{r}
# check if the projected and geographic projections match 
...
if(st_crs(dataset1) != st_crs(dataset2)) { 
  warning("coordinate sustems don't match")
...
```{r}
# See number of rows each data has before joining 
nrow(coffee_data)
...
tm_shape(world_coffee) + 
  tm_fill("coffee_production_2017", 
          fill.legend = tm_legend(title = "coffee_production_2017"))
...
"coffee_production_2017" <- inner_join(world, coffee_data, by = "name_long")
```

Let's build ourselves a warning message to make sure we don't lose any data because of incomplete matches.

```{r}
#Warning message if rows match but names are different so R doesn't thinl they match 

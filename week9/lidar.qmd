...
dsm_pre <- rast(here::here("week9", "data", "Eaton2", "dsm_eaton_alignedNK_1m_PREFIRE.tif"))
  
  
dtm_post <- rast(here::here( "week9", "data","Eaton2", "dtm_eaton_epsg32611_geoid12b_1m_POSTFIRE.tif"))
dsm_post <- rast(here::here( "week9", "data", "Eaton", "dsm_eaton_epsg32611_geoid12b_1m_POSTFIRE.tif"))
```

```{r}
damage <- st_read(here::here("week9","data","geospatial.gdb")) %>% st_transform(crs = crs(dsm_post))
```


```{r}

eaton_perimeter <- st_read(here::here( "week9", "data", "Eaton2","Eaton_Perimeter_20250121", "Eaton_Perimeter_20250121.shp"))%>% st_transform(crs = crs(dsm_post))
```

```{r}
# All crs match... didnt type big long if statement 
if (crs(dsm_post) == crs(dsm_pre) & 
    crs(dtm_pre) == crs(dtm_post) & 
    crs(dtm_pre) == crs(damage)) {
  print("All CRS match")
} else {
...
```{r}
# Crop Surcace chanhe to extend of eaton perimeter 
...
quantiles <- global(diff_dsm_crop, fun = quantile, 
                    probs = c(0.005, 0.995), # Taking outliars out 
                    na.rm = TRUE)

# Look at quantiles to see lower and upper bounds 

# Remove calues outside of upper and lower quantiles 
...
tm_shape(diff_dsm_scaled) + 
  ...
  tm_borders(col = 'red', 
             lwd = 2.5) + 
  tm_title("Easton Fire: DSM Elevation Change (2025- 2016)") + 
  tm_credits("Data: OpenTopograohy", 
...
dtm_crop <- crop(dtm_post, neighborhood) # Elevation AFTER the fire 
...
```{r}
# This wont work because rayshader blows my computer up 
elev_mat <- raster_to_matrix(dtm_change)
diff_mat <- raster_to_matrx(diff_crop)

overlay <- height_shade(
   diff_mat, 
   texture = colorRampPalette(c("#8B0000", "#FF6347", "#FFFFFF", 
                   "#87CEEB", "#00008B"))(256), # Pick out 256 colors and match those to elevation 
   range = c(-20, 20) # Levels of brightness between these scales
   )
 
 # Create map 
elev_mat %>% 
   Create a black and white hillshape effect 
   sphere_shade(texture = "bw", sunangle = 315) %>% 
   add_overlay(alphalayer = 0.7) %>% 
    add realostic shadows at 50% intensity to enhance the 3D effect 
...
# Extract the min elevation loss for each buffer 
lidar_min <- terra::extract(diff_dsm, damafge_buffer, fun = min, na.rm = TRUE)
```


```{r}
damge_analysis <- damage_buffer %>% 
  mutate(lidar_min_change = lidar_min[[2]], 
         elevation_loss = abs(lidar_min_change)) # Meters of elevation loss 

```


```{r}
# Create a plot of elecvation loss based on year built 
damage_plot <- damge_analysis %>% 
  st_drop_geometry() %>% 
  filter(!is.na(YEARBUILT)) %>% 
  mutate( 
    
    age_cataegory = factor(case_when(
      
      YEARBUILT >= 2000 ~"2000+", 
      YEARBUILT >= 1980 ~"1980-1999", 
      YEARBUILT >= 1960 ~"1960-1989", 
...
```{r}
ggplot(damage_plot, aes(x = age_cataegory, y = elevation_loss, fill = age_cataegory)) + 
  geom_boxplot(alpha = 0.7) + 
  scale_fill_brewer(palette = "YlOrRd") + 
  coord_flip() + 
  labs(
    title = "Building time period vs elevation loss", 
   x = "Buidling time period", 
   y = "Elevation loss(m)"
  ) + 
  theme_minimal() + 

...
```{r}
nz <- spData::nz # polygons 
nz_height <- spData::nz_height # Elevation- points 


# filter to the region on Cantery Berry 
canterbury <- nz %>% 
...
```{r}
# Filtering for all rows that have "canterbury in it 
...
# check that interect is opposite of disjoint 
...
```{r}
# Subset to efatures in Canterbury with st filter 
c_height2 <- nz_height %>% 
...
# If intersect, returns 1. If not intersects, returns empty <- 
nz_height_sgbp <- st_intersects(x = nz_height, y = canterbury)
nz_height_sgbp

# Return to logical vector 
na_height_logical <- lengths(nz_height_sgbp) > 0 # returns boolean operator 
na_height_logical

# filter based on logical vector 
c_height3 <- nz_height[na_height_logical, ]
```

Now let's plot results from all three methods to confirm they gave the same results.

```{r}
# Looking at plots in which we filtered differently 
...
```{r}
# Find heights within 1000 km of Canterbury (1000km from the border of Canterbury)
nz_height_logical <- st_is_within_distance(nz_height, canterbury, 
                                           dist = units::set_units(1000, "km"), 
                                           sparse = FALSE) # return as logical

# Filter based on logical vector 
c_heights4 <- nz_height[nz_height_logical, ] # looking a rows that intersect with the logical df 
...
```{r}
# LEFT join: keeping all rows from nz_height and adding nz data to it 
nz_height_left_join <- st_join(nz_height, nz) %>% 
  select(id = t50_fid, elevation, region = Name) # renamed columns 
...
```{r}
# Check with view map to see relationship / intersected-ness of the 2 dataset points 
tmap_mode("view")
...
cycle_hire_join <- st_join(cycle_hire, cycle_hire_osm, 
                           ...
                           dist = units::set_units(20, "m")) %>% # join if points are 20m of eachother
  select(id, capacity)
...
```{r}
# Want an average of capacities per 1 observarion: 
...
```{r}
# Produce a warning message if the data does not match 
...
```{r}
# aggregated values for a single point in cycle_hire_join 
...
```{r}
# GOAL: take the average elevation for each location  

# Method 1: join + group by + summarise 
...
```{r}
# INCONGRUENT!! - Data doesn't perfectly match 
# Need to interpolate! 
...
```{r}
# select just the values we want to aggregate 
incongruent_2 <- incongruent %>% select(value)

# Use area-weighted interpolation to aggregate the "value" attribute 

aggregateing_zones_area_weighted <- st_interpolate_aw(x = incongruent_2, 
                                                      aggregating_zones, 
                                                      extensive = TRUE)

# x = areas we want to interpolate onto a broader level 
# extensive = TRUE ... aggregating a population or count value 
# extensive = FALSE ... aggregating with density and rate  


# aggregated all the blue line data (9) into 2 areas 
aggregateing_zones_area_weighted$value
```
:::
...
# summarise total population within each region 
# METHOD 1: group by and summarise 
regions1 <- us_states %>% 
  group_by(REGION) %>% 
...
# METHOD 2: aggregate 
regions2 <- aggregate(us_states[, "total_pop_15"],  # x = Data to be aggregated [all rows, tot pop col]
                      by = list(us_states$REGION), # by = Need to be a list 
                      FUN = sum, na.rm = TRUE # rhe function 
                      )
...
```{r}
# finding the centriod = center of the data 
